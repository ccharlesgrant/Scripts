    ...
    sendBtn.onclick = () => {
        ...
        document.querySelectorAll("#village_troup_list tbody tr.call-village").forEach(row => {
            ...
            ["spear","sword","archer","spy","heavy"].forEach(u => {
                ...
                const avail = Math.max(0, total - res); // strict
        ...
        villages.forEach(v => {
            ...
            DEFENSE_UNITS.forEach(u => {
                ...
                // Strict guard: if no available troops → send ZERO
                if (v.availTroops[u] > 0) {
                    const proportion = v.availTroops[u] / totalDefenseThis;
                    const raw = proportion * packetDefenseCap;
                    amt = Math.trunc(raw); // integer truncation
                    amt = Math.min(amt, v.availTroops[u]); // never exceed
                }

                // Final enforcement: if available <= 0, force 0 (covers all edge cases)
            ...
            v.packet = packet;
            v.packetPop = packetDefensePop; // defense only (excl. scouts)
        });
