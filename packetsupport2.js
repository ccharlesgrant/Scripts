...
Array.from(document.querySelectorAll("#village_troup_list tbody tr")).forEach(row => {
    // More flexible coordinate finding
    let villageCell = row.querySelector(".village, td:first-child, [style*='cursor:pointer']");
    let coordMatch = villageCell?.textContent?.match(/(\d+\|\d+)/);
    if (!coordMatch) return;
    let coord = coordMatch[1];

    let distanceCell = row.querySelector(".distance, td:nth-child(2)");
    let distance = parseFloat(distanceCell?.textContent) || 999;

    let troops = {};
    let availablePop = 0;

    DEFENSE_UNITS.forEach(unit => {
        // More ways to find the cell
        let cell = row.querySelector(`[data-unit="${unit}"], .unit-${unit}, td[data-unit*="${unit}"], img[src*="unit_${unit}"] + span, img[src*="unit_${unit}"] ~ span`);
        let totalStr = cell?.textContent?.trim() || "0";
        // Clean common formatting (12.3k → 12300)
        let total = parseFloat(totalStr.replace(/k/i, "000").replace(/[.,]/g, "")) || 0;
    ...
    if (availablePop > 500) {  // small safety threshold
        villages.push({ coord, distance, troops, availablePop });
